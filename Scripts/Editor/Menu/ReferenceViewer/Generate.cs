...
namespace ReferenceViewer
{
    public class Generate
    {
        ...
        public static void Build(string[] assetPaths, Action<AssetData[]> callback = null)
        {
            ...
            for (var i = 0; i < assetPaths.Length; i++)
            {
                ...
                switch (Path.GetExtension(assetPath))
                {
                    ...
                    case ".unity":
                        DisplayProgressBar(assetData.path, progress);
                        if (UnityEditor.SceneManagement.EditorSceneManager.OpenScene(assetPath).IsValid())
                        {
                    ...
                    case ".controller":
                        UnityEditor.Animations.AnimatorController animator =
                            (UnityEditor.Animations.AnimatorController)
                                AssetDatabase.LoadAssetAtPath(assetPath, typeof(UnityEditor.Animations.AnimatorController));
                        var layerCount = animator.layers.Length;
						//if (isUnity41)
						//{
                        ...
                        for (var j = 0; j < layerCount; j++)
                        {
							...
							stateMachine = animator.layers[j].stateMachine;
							//if (isUnity41)
							//{
							//	stateMachine =
							...
							foreach( var s in stateMachine.stateMachines )
								SearchMotion(animator, s.stateMachine, assetData);
        ...
        private static void SearchMotion(UnityEditor.Animations.AnimatorController animator, UnityEditor.Animations.AnimatorStateMachine stateMachine, AssetData assetData)
        {
			//var stateCount = 0;
			...
			foreach( var stm in stateMachine.states )
            {
				var motion = stm.state.motion;
        ...
        private static void SearchBlendTreeMotion(UnityEditor.Animations.AnimatorController animator, Motion motion, AssetData assetData)
        {
			...
			//}
			foreach(var bt in blendTree.children)
			{
				SearchBlendTreeMotion(animator, bt.motion , assetData);
			}
        }
        ...
        private static void SearchProperty(Object component, object val, AssetData assetData, bool isScene)
        {
            ...
            foreach (var info in properties.Where(info => info.CanRead))
            {
				...
				// rigidbodyの変換でエラーを吐いていたのでtry catch追加
				try
				{
	                value = info.GetValue(val, new object[0]);
				}
				catch
				{
