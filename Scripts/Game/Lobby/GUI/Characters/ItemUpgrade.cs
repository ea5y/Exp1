...
namespace XUI
{
    public class ItemUpgrade : CustomControl.ScrollViewItem
    {
        ...
        public override void FillItem(IList datas, int index)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                if (charasDataBuffer.Count != 0)
                {
                    ...
                    //set fodder type
                    characters[j].charaType = charasDataBuffer[0].UpgradeType;

                    //Set hook
                    characters[j].hook.gameObject.SetActive(charasDataBuffer[0].selectFlagUpgrade);
                    ...
                    //set cost
                    characters[j].costGold = foodData.LevelUpCostGold;
